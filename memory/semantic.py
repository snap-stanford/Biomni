...
class SemanticMemoryStore:
    ...
    def _upsert_fact(self, session: Session, memory_id: uuid.UUID | str, fact: MemoryFact) -> Fact:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        "save_facts chunk commit failed (attempt %d/%d): %s; retrying in %.2fs",
                        attempt + 1,
                        max_retries + 1,
                        exc,
                        delay,
                    ...
                    continue
                logger.warning(
                    "save_facts chunk commit failed after %d retries: %s; falling back to per-fact persist",
                    max_retries,
                    exc,
                )
    ...
    def query_fact(self, entity: str | None = None, relation: str | None = None, limit: int = 50) -> list[dict]:
    ...
    def get_active_facts_by_memory(self, memory_id: uuid.UUID | str) -> list[dict]:
        ...
        stmt = select(Fact).where(Fact.memory_id == uuid.UUID(str(memory_id)), Fact.status == "active")
        with self.session_factory() as session:
            rows = session.scalars(stmt).all()
            return [self._to_dict(r) for r in rows]

    def get_active_facts_by_memories(self, memory_ids: Sequence[uuid.UUID | str], user_id: str) -> list[dict]:
    ...
    def update_fact_feedback(self, fact_id: uuid.UUID | str, feedback: int, user_id: str) -> Fact | None:
    ...
    def list_memories_without_active_facts(self) -> list[uuid.UUID]:
        ...
        with self.session_factory() as session:
            stmt = (
                ...
                .where(Memory.id.not_in(select(Fact.memory_id).where(Fact.status == "active")))
            )
