...
def _markdown_to_flowables(markdown_text: str, styles) -> list:
    ...
    for part in parts:
        ...
        def _flush_lists():
            ...
            if list_buffer:
                items = [
                    ListItem(Paragraph(_convert_inline_markdown_to_html(item), styles["Normal"]))
            ...
            if ordered_list_buffer:
                items = [
                    ListItem(Paragraph(_convert_inline_markdown_to_html(item), styles["Normal"]))
                    for item in ordered_list_buffer
                ]
...
def generate_pdf_report(
    ...
    styles.add(
        ParagraphStyle(
            "Code", parent=styles["Normal"], fontName="Courier", fontSize=9, leading=11, backColor=colors.whitesmoke
        )
    )
    ...
    if chat_history:
        ...
        for msg in chat_history:
            ...
            # Add figures
            for _fig_label, fig_path in figures:
                try:
                    ...
                    story.append(
                        Paragraph(f"<i>[Figure not available: {fig_path.name}: {str(e)}]</i>", styles["Normal"])
                    )
