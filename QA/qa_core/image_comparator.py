...
class ImageComparator:
    ...
    def verify_images_exist(self, image_refs: list[str], task_dir: Path) -> tuple[list[str], list[str]]:
        """
        이미지 파일 존재 확인
    ...
    def evaluate_images(
        ...
        [img for img in referenced_images if img in found_images]
        ...
        if use_llm_comparison and self.llm_client and expected_images and found_images:
            ...
            # 생성된 이미지 경로들
            generated_image_paths = [generated_task_dir / img_name for img_name in found_images]
    ...
    def _create_image_comparison_prompt(self, question: str | None = None, image_name: str | None = None) -> str:
        """
    ...
    def _parse_llm_multi_image_response(self, response: str) -> dict:
        ...
        try:
            ...
            content_match = re.search(r"all_content_present[:\s]+(true|false)", response, re.IGNORECASE)

            score = float(score_match.group(1)) if score_match else 0.0
            all_content_present = content_match.group(1).lower() == "true" if content_match else False
            passed = score >= self.llm_threshold
