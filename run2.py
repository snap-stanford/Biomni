...
for data_file in data_files:
    parent_data_path = os.path.join(original_dir, data_file)
    if os.path.exists(parent_data_path):
        shutil.copy2(parent_data_path, data_file)
        print(f"✅ Copied {data_file} to current directory")
    elif not os.path.exists(data_file):
        print(f"⚠️ Warning: {data_file} not found in current or parent directory")
...
user_command = '''
Perform a SAR (Structure-Activity Relationship) analysis on the uploaded `molecules.csv` file.
'''
# user_command = f"""
