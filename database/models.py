...
class Memory(Base):
    ...
    facts: Mapped[list[Fact]] = relationship(back_populates="memory", cascade="all, delete-orphan", lazy="selectin")
...
class Fact(Base):
    ...
    # Lifecycle / scoring columns. `created_at`/`updated_at` are generated by the
    # database, never the LLM. `access_count` is a retrieval side-effect counter.
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    ...
    status: Mapped[str] = mapped_column(String(255), nullable=False, default="active", server_default=text("'active'"))
    access_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default=text("0"))
    # Explicit user feedback, kept separate from `access_count` (retrieval usage).
    # Feedback is a deliberate user judgement (helpful / wrong), never auto-derived
    # from the fact merely being retrieved. Counts stay >= 0 and never imply delete.
    positive_feedback_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default=text("0"))
