...
if execute_button:
    # Create integrated analysis prompt - SIMPLIFIED (Agent-Trusting Approach)
    ...
    if analysis_steps:
        ...
        prompt = f"""Perform bioinformatics analysis following this methodology:
...
2. Data briefing shows patterns (e.g., "TCGA-* (600 samples)")
3. Use df.columns to get actual column names
4. For data columns: use exact names listed
5. For sample columns: use df.columns.tolist() to get actual names
...
- Provide comprehensive results in <solution> tag
- Include specific numbers and statistics in your observations
...
⚠️ CRITICAL: Verify column names with df.columns.tolist() before any operations.

METHOD:
{method_preview}

Save plots as PNG files. Provide comprehensive results in <solution> tag."""
