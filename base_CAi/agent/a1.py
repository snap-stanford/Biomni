...
from base_CAi.config import default_config
from base_CAi.know_how import KnowHowLoader
from base_CAi.llm import SourceType, get_llm
from base_CAi.model.retriever import ToolRetriever
...
class A1:
    def __init__(
        ...
        if commercial_mode:
            from base_CAi.env_desc_cm import data_lake_dict, library_content_dict

            print("🏢 Commercial mode: Using commercial-licensed datasets only")
        else:
            from base_CAi.env_desc import data_lake_dict, library_content_dict
        ...
        data_lake_dir = os.path.join(path, "base_CAi_data", "data_lake")
        ...
        self.path = os.path.join(path, "base_CAi_data")
    ...
    def add_tool(self, api):
        ...
        try:
            ...
            builtins._base_CAi_custom_functions[schema["name"]] = api
    ...
    def add_mcp(self, config_path: str | Path = "./tutorials/examples/mcp_config.yaml") -> None:
        ...
        the base_CAi agent system. Each MCP server is loaded as an independent module
        ...
        for server_name, server_meta in mcp_servers.items():
            ...
            for tool_meta in tools_config:
                if isinstance(tool_meta, dict) and "base_CAi_name" in tool_meta:
                    # Manual tool definition
                    tool_name = tool_meta.get("base_CAi_name")
...
"""
        ...
        # Add drug discovery specific instructions
        prompt_modifier += """
DRUG DISCOVERY GUIDELINES:
- For molecular property prediction (ADMET, toxicity, solubility), prefer established tools like RDKit, DeepPurpose, or pytdc.
- For synthesis planning and retrosynthesis, use SCScore to evaluate synthetic accessibility before proposing routes.
- For molecular docking, use AutoDock Vina or ADFR; always validate binding poses with multiple metrics (docking score, RMSD, interactions).
- For molecule generation, clearly report validity rate, uniqueness, and novelty of generated molecules.
- When working with SMILES strings, always validate them with RDKit before passing to downstream tools.
- For literature search on drug targets or compounds, use advanced_web_search_claude() or query_pubmed().
- Prioritize accuracy over speed; always report confidence levels and known limitations of predictions.
"""
...
- Drug Discovery Data Lake
You can access a drug discovery data lake at the following path: {data_lake_path}.
{data_lake_intro}
...
- Note on using R packages and Bash scripts:
  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def create_mcp_server(self, tool_modules=None):
        """
        Create an MCP server object that exposes internal base_CAi tools.
            ...
            try:
                ...
                for tool_schema in module_tools:
                    ...
                    try:
                        ...
                        wrapper_func = self._generate_mcp_wrapper_from_base_CAi_schema(
    ...
    def _generate_markdown_content(self, include_images: bool = True) -> str:
        ...
        content = """# base_CAi Agent Conversation History
...
"""
    ...
    def _clear_execution_plots(self):
        ...
        """
        try:
            from base_CAi.tool.support_tools import clear_captured_plots
