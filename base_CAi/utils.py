...
'required_parameters': [{{'name': 'param1', 'type': 'str', 'description': 'Parameter description', 'default': None}}],
'optional_parameters': [{{'name': 'param2', 'type': 'int', 'description': 'Parameter description', 'default': 10}}]}}

Rules:
- For parameters without default values, set 'default': None
- For parameters with default values, include them in 'optional_parameters'
- Use Python types: str, int, float, bool, List[str], dict, etc.
- Use capitalized True/False/None (Python style, not JSON)
- Keep descriptions clear and concise
- Do NOT create fake optional parameters - only use what's in the function signature
- Return ONLY the dictionary, no markdown code blocks or extra text
...
"""
    ...
    for attempt in range(5):
        ...
        try:
            ...
            if attempt < 4:  # Don't print on last attempt
                ...
                if api_str:
                    display_str = api_str[:150] + "..." if len(api_str) > 150 else api_str
                    print(f"{error_msg}. Output: {display_str}")
                else:
    ...
    print("ERROR: Could not generate valid API schema after 5 attempts")
    return "Error: Could not parse the API schema after multiple attempts"
...
def api_schema_to_langchain_tool(api_schema, mode="generated_tool", module_name=None):
    if mode == "generated_tool":
        module = importlib.import_module("base_CAi.tool.generated_tool." + api_schema["tool_name"] + ".api")
...
def read_module2api():
    ...
    module2api = {}
    for field in fields:
        module_name = f"base_CAi.tool.tool_description.{field}"
        module = importlib.import_module(module_name)
        module2api[f"base_CAi.tool.{field}"] = module.description
    return module2api
...
def check_and_download_s3_files(
    ...
    Args:
        s3_bucket_url: Base URL of the S3 bucket (e.g., "https://base_CAi-release.s3.amazonaws.com")
...
def parse_tool_calls_from_code(code: str, module2api: dict, custom_functions: dict = None) -> list[str]:
    ...
    Example:
        >>> code = "from base_CAi.tool import analyze_data\nimport pandas as pd"
...
def inject_custom_functions_to_repl(custom_functions: dict):
    ...
    if custom_functions:
        ...
        from base_CAi.tool.support_tools import _persistent_namespace
        ...
        builtins._base_CAi_custom_functions.update(custom_functions)
...
def format_detected_tools(detected_tool_modules: list) -> str:
    ...
    Example:
        >>> format_detected_tools([("analyze_data", "base_CAi.tool"), ("pandas", "pandas")])
        "base_CAi → analyze_data, pandas → pandas"
    """
    tool_descriptions = []
...
def remove_emojis_from_text(text: str) -> str:
    ...
    Note:
        The function targets specific emojis used in the base_CAi system:
        - 🔧 for tools
        - 📊 for data
        - ⚙️ for software
        - 📋 for configuration
...
def convert_markdown_to_pdf(markdown_path: str, pdf_path: str) -> None:
    ...
    try:
        ...
        <head>
            <meta charset="utf-8">
            <title>base_CAi Conversation History</title>
