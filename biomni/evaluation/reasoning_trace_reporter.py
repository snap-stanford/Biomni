...
class ReasoningTraceReporter:
    ...
    def _parse_log_entry(self, log_entry: str, step_index: int):
        ...
        elif any(error_marker in cleaned_entry for error_marker in [
            "NSInternalInconsistencyException", "NSWindow", "libc++abi: terminating",
            "matplotlib", "GUI", "thread", "abort"
        ]):
            self._extract_error(cleaned_entry, step_index)
        
        # Look for planning and checklist patterns
        elif any(planning_marker in cleaned_entry.lower() for planning_marker in [
            "plan checklist", "updated plan", "step", "checklist", "plan:", "steps:"
        ]):
            self._extract_planning(cleaned_entry, step_index)
        
        # Look for thinking/reasoning patterns in unstructured text
        elif any(thinking_marker in cleaned_entry.lower() for thinking_marker in [
            "i need to", "let me", "first, i'll", "i should", "i think", "i believe",
            "based on", "considering", "looking at", "analyzing", "examining"
        ]):
        ...
        elif any(code_marker in cleaned_entry for code_marker in [
            "def ", "import ", "class ", "for ", "if ", "while ", "try:", "except:"
        ]):
            self._extract_code_generation(cleaned_entry, step_index)
