...
def perform_multi_group_test(
    ...
    >>> # Define groups
    >>> groups = pd.Series(
    ...     ["Control", "Control", "Treatment1", "Treatment1", "Treatment2", "Treatment2"], index=data.columns
    ... )
    >>>
    >>> # Perform ANOVA (parametric)
    >>> results = perform_multi_group_test(data, data.columns.tolist(), groups, method="one_way", post_hoc="tukey")
    >>>
    >>> # Perform Kruskal-Wallis (non-parametric)
    >>> results = perform_multi_group_test(data, data.columns.tolist(), groups, method="kruskal", post_hoc="bonferroni")
    >>>
            ...
            try:
                ...
                for feature_idx in results[results["significant"]].index:
                    try:
                        ...
                        data.loc[feature_idx, sample_columns].values

                        # Create long format data
                        tukey_data = []
                        tukey_groups = []
                        for sample, group in zip(sample_columns, groups, strict=False):
...
def perform_survival_analysis(
    ...
    >>> np.random.seed(42)
    >>> n_patients = 50
    >>> expression = pd.DataFrame(np.random.randn(10, n_patients), columns=[f"Patient{i}" for i in range(n_patients)])
    >>> expression.index = [f"Gene{i}" for i in range(10)]
    >>>
    >>> # Simulate survival data
    >>> survival_time = pd.Series(np.random.exponential(scale=365, size=n_patients), index=expression.columns)
    >>> event = pd.Series(np.random.binomial(1, 0.6, size=n_patients), index=expression.columns)
    >>>
    >>> # Perform survival analysis
    >>> results = perform_survival_analysis(expression, survival_time, event, method="cox")
    >>>
    ...
    if method == "cox":
        ...
        for feature in expression_data.index:
            try:
                ...
                cox_data = pd.DataFrame({"time": survival_time, "event": event, "expression": feature_expr})
        ...
        for feature in expression_data.index:
            try:
                ...
                # Prepare data
                test_data = pd.DataFrame({"time": survival_time, "event": event, "group": groups})
                ...
                # Log-rank test
                result = multivariate_logrank_test(test_data["time"], test_data["group"], test_data["event"])
...
def plot_kaplan_meier(
    ...
    >>> np.random.seed(42)
    >>> n = 100
    >>> survival_time = pd.Series(np.random.exponential(scale=365, size=n), index=[f"Patient{i}" for i in range(n)])
    >>> event = pd.Series(np.random.binomial(1, 0.6, size=n), index=survival_time.index)
    >>>
    >>> # Create groups (high vs low expression)
    >>> groups = pd.Series(["High" if i < 50 else "Low" for i in range(n)], index=survival_time.index)
    >>>
    >>> # Plot Kaplan-Meier curves
    >>> plot_path = plot_kaplan_meier(survival_time, event, groups=groups, output_file="km_plot.png")
    ...
    if risk_table:
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), gridspec_kw={"height_ratios": [3, 1]}, facecolor="white")
    else:
    ...
    if groups is None:
        ...
        kmf.plot_survival_function(ax=ax1, ci_show=confidence_interval, color="#3498DB", linewidth=2.5)
        ...
        for i, group in enumerate(unique_groups):
            mask = groups == group
            kmf.fit(survival_time[mask], event[mask], label=f"{group} (n={mask.sum()})")
            kmf.plot_survival_function(ax=ax1, ci_show=confidence_interval, color=colors[i], linewidth=2.5)
        ...
        if len(unique_groups) == 2:
            ...
            result = logrank_test(
                survival_time[group1_mask], survival_time[group2_mask], event[group1_mask], event[group2_mask]
            )
    ...
    ax1.set_title("Kaplan-Meier Survival Curves", fontsize=16, fontweight="bold", color="#2C3E50", pad=15)
...
def adjust_pvalues(
    pvalues: list[float] | np.ndarray | pd.Series, method: str = "fdr_bh", alpha: float = 0.05
    ...
    if method == "fdr_bh":
        ...
        for i in range(n - 1, -1, -1):
            if i == n - 1:
                ...
                padj[sorted_idx[i]] = min(sorted_pvals[i] * n * cm / (i + 1), padj[sorted_idx[i + 1]], 1.0)
        ...
        padj = np.zeros(n)
        for i in range(n):
            padj[sorted_idx[i]] = min(sorted_pvals[i] * (n - i), 1.0)
            if i > 0:
...
def t_test_FDR(
    ...
    Examples
    --------
    >>> data = pd.DataFrame({"C1": [10, 20, 30], "C2": [12, 22, 32], "T1": [15, 25, 35], "T2": [17, 27, 37]})
    ...
    results = pd.DataFrame(
        {"statistic": t_stats, "p_value": p_values, "mean_group1": mean1, "mean_group2": mean2, "fold_change": log2_fc},
        index=data.index,
    )
...
def calculate_correlation(
    ...
    for i, feat1 in enumerate(data1.index):
        ...
        for j in range(start_j, len(data2.index)):
            ...
            results.append(
                {"feature1": feat1, "feature2": feat2, "correlation": corr, "p_value": pval, "n_samples": n_samples}
            )
    ...
    if adjust_p and len(results_df) > 0:
        ...
        results_df["p_value_adj"] = adjust_pvals_func(results_df["p_value"].values, method="fdr_bh")
...
def perform_permutation_test(
    ...
    # Create results
    results = pd.DataFrame({"observed_stat": observed_stats, "p_value": p_values}, index=data.index)
...
def perform_nonparametric_test(
    ...
    ...     data, group1_samples=["s1", "s2", "s3"], group2_samples=["s4", "s5"], test="mannwhitney"
    ... )
    >>>
    ...
    # Create results
    results = pd.DataFrame({"statistic": test_stats, "p_value": p_values}, index=data.index)
...
def test_variance_homogeneity(
    data: pd.DataFrame, sample_groups: dict[str, list[str]], test: str = "levene"
) -> dict[str, any]:
    """
    ...
    Examples
    --------
    >>> groups = {"Control": ["C1", "C2", "C3"], "Treatment": ["T1", "T2", "T3"]}
    ...
    group_data = []
    for _group_name, sample_list in sample_groups.items():
...
def check_test_assumptions(
    data: pd.DataFrame, sample_groups: dict[str, list[str]], paired: bool = False, verbose: bool = True
) -> dict[str, any]:
    """
    ...
    Examples
    --------
    >>> groups = {"Control": ["C1", "C2", "C3", "C4"], "Treatment": ["T1", "T2", "T3", "T4"]}
    ...
    if test_normality_func is not None:
        try:
            normality_results = test_normality_func(data, all_samples, test="shapiro", sample_size=min(100, len(data)))
            if len(normality_results) > 0:
                ...
                if normality_pct < 50:
                    warnings.append(
                        f"Most features non-normal ({normality_pct:.1f}% normal). Non-parametric tests recommended."
                    )
                elif normality_pct < 70:
                    warnings.append(
                        f"Borderline normality ({normality_pct:.1f}% normal). Consider both parametric and non-parametric tests."
                    )
        except:
            pass
...
def recommend_statistical_test(
    ...
    if analysis_type == "differential":
        ...
        if is_normal:
            ...
            confidence = 0.85
            rationale = (
                f"Normal distribution ({normality_pct:.1f}% features). Pearson correlation for linear relationships."
            )
...
def smart_differential_analysis(
    ...
    >>>
    >>> # Force specific test
    >>> results, metadata = smart_differential_analysis(data, groups, auto_select_test=False, force_test="mannwhitney")
    ...
    try:
        if n_groups == 2:
            ...
            if selected_test in ["Welch's t-test", "welch", "ttest"]:
                ...
                results = t_test_FDR(
                    data.copy(), group1_samples, group2_samples, equal_var=False, adjust_p=adjust_pvalues, alpha=alpha
                )
                test_function = "t_test_FDR"

            elif selected_test in ["Mann-Whitney U test", "mannwhitney"]:
                results = perform_nonparametric_test(
                    data, group1_samples, group2_samples, test="mannwhitney", adjust_p=adjust_pvalues
                )
                test_function = "perform_nonparametric_test(test='mannwhitney')"

            elif selected_test in ["Wilcoxon signed-rank test", "wilcoxon"]:
                results = perform_nonparametric_test(
                    data, group1_samples, group2_samples, test="wilcoxon", adjust_p=adjust_pvalues, paired=True
                )
                test_function = "perform_nonparametric_test(test='wilcoxon', paired=True)"

            elif selected_test in ["Permutation test", "permutation"]:
                results = perform_permutation_test(data, group1_samples, group2_samples, n_permutations=1000)
                ...
                results = pd.DataFrame({"statistic": t_stats, "p_value": p_vals}, index=data.index)
