        ...
        if input_path.endswith(('.nii', '.nii.gz')):
            try:
                ...
                if len(img.shape) == 4 and img.shape[3] == 4:
                    print(f"4D NIfTI file detected, splitting modalities...")
                    return split_modalities(input_path, output_dir, case_name)
                else:
                    print(f"Single 3D file detected, copying to output directory...")
...
def segment_with_nn_unet(image_path, output_dir, task_id, model_type='3d_fullres', folds=[0, 1, 2, 3, 4], use_tta=False, num_threads=1, mixed_precision=True, verbose=True, auto_prepare_input=True, results_folder=None):
    """
