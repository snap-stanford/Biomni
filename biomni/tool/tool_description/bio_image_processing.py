...
MANUAL_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "quantify_bands": {
        ...
        "parameters": {
            ...
            "rois": {
                "description": ("Sequence of ROI tuples in `(x, y, width, height)` format defining band locations."),
                "type": "Sequence[Sequence[float]]",
            },
            "background_width": {
                "description": ("Number of pixels to expand around each ROI when gathering background samples."),
                "type": "int",
                "default": 3,
            },
            "back_pos": {
                "description": ("Background sampling strategy. Choose among `all`, `top/bottom`, or `sides`."),
                "type": "str",
                "default": "all",
            },
            "back_type": {
                "description": ("Aggregation method for background pixels. Either `mean` or `median`."),
                "type": "str",
                "default": "median",
            },
        },
    ...
    "binarize_image": {
        ...
        "parameters": {
            "image": {
                "description": ("Input image as numpy array or absolute/relative path to the image file."),
                "type": "np.ndarray | str",
            },
            "threshold": {
                "description": (
...
def _iter_positional_args(
    ...
    defaults: list[ast.expr | None] = [None] * (len(pos_args) - len(node.args.defaults)) + list(node.args.defaults)
    return zip(pos_args, defaults, strict=False)


def _iter_kwonly_args(
    node: ast.FunctionDef,
) -> Iterable[tuple[ast.arg, ast.expr | None]]:
    return zip(node.args.kwonlyargs, node.args.kw_defaults, strict=False)


def _merge_manual_parameter_override(parameter: dict[str, Any], override: dict[str, dict[str, Any]]) -> None:
...
def get_description(filepath: str) -> list[dict[str, Any]]:
    ...
    for node in tree.body:
        ...
        for kw_arg, default in _iter_kwonly_args(node):
            ...
            parameter_entries.append(_build_parameter_entry(kw_arg, default, doc_param))
...
MODULE_PATH = (Path(__file__).resolve().parent.parent / "bio_image_processing.py").resolve()
