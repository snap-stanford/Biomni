...
# ChEMBL Beaker Integration Functions


class ChEMBLBeakerClient:
    """
    Client for interacting with ChEMBL Beaker cheminformatic utilities.
    
    Provides access to chemical structure standardisation, physico-chemical property 
    calculation, format converters, and other cheminformatic tools via the ChEMBL Beaker API.
    """
    ...
    def _handle_rate_limiting(self):
        """Implement rate limiting to respect ChEMBL API limits."""
        current_time = self.time.time()
        time_since_last = current_time - self.last_request_time
    ...
    def _make_request(self, endpoint: str, data: dict = None, files: dict = None) -> dict:
        """Make API request with retry logic and error handling."""
        self._handle_rate_limiting()
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if response.headers.get('content-type', '').startswith('application/json'):
                    ...
                    return {"result": response.text, "content_type": response.headers.get('content-type')}
                    
            except self.requests.exceptions.Timeout:
                if attempt == self.retry_attempts - 1:
                    raise Exception("ChEMBL Beaker API request timed out after multiple attempts")
                self.time.sleep(2 ** attempt)
                ...
                if e.response.status_code == 429:
                    ...
                    return {"error": f"Validation error: {e.response.text}", "status_code": 422}
                else:
                    raise Exception(f"ChEMBL Beaker API HTTP Error: {e.response.status_code} - {e.response.text}")
                    
            except Exception as e:
                if attempt == self.retry_attempts - 1:
                    raise Exception(f"ChEMBL Beaker API request failed: {str(e)}")
                self.time.sleep(2 ** attempt)
        
        return {}
    
    def check_structure(self, molfile_content: str) -> dict:
        """Check a molecule for issues using ChEMBL Structure Pipeline functionality."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("check", files=files)
    
    def exclude_structure(self, molfile_content: str) -> dict:
        """Check if the structure will be excluded from ChEMBL."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("exclude", files=files)
    
    def get_parent_structure(self, molfile_content: str) -> dict:
        """Remove salt/solvates using ChEMBL Structure Pipeline functionality."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("getParent", files=files)
    
    def standardize_structure(self, molfile_content: str) -> dict:
        """Standardise a molecule using ChEMBL Structure Pipeline functionality."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("standardize", files=files)
    
    def get_chembl_descriptors(self, molfile_content: str) -> dict:
        """Get selected physico-chemical descriptors available in ChEMBL."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("chemblDescriptors", files=files)
    
    def get_all_descriptors(self, molfile_content: str) -> dict:
        """Get all physico-chemical descriptors calculated by RDKit."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("descriptors", files=files)
    
    def canonicalize_smiles(self, smiles: str) -> dict:
        """Convert SMILES to canonical SMILES."""
        data = {"smiles": smiles}
        return self._make_request("canonicalizeSmiles", data=data)
    
    def generate_2d_coordinates(self, molfile_content: str) -> dict:
        """Generate 2D coordinates for a molecule using Schrodinger's coordgen."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("ctab22D", files=files)
    
    def ctab_to_inchi(self, molfile_content: str) -> dict:
        """Convert CTAB to InChI."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("ctab2inchi", files=files)
    
    def ctab_to_inchikey(self, molfile_content: str) -> dict:
        """Convert CTAB to InChIKey."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("ctab2inchikey", files=files)
    
    def ctab_to_smarts(self, molfile_content: str) -> dict:
        """Convert CTAB to SMARTS format."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("ctab2smarts", files=files)
    
    def ctab_to_smiles(self, molfile_content: str) -> dict:
        """Convert CTAB to SMILES format."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("ctab2smiles", files=files)
    
    def ctab_to_svg(self, molfile_content: str, size: int = 300) -> dict:
        """Convert CTAB to SVG vector graphic."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        data = {"size": str(size)}
        return self._make_request("ctab2svg", data=data, files=files)
    
    def inchi_to_ctab(self, inchi: str) -> dict:
        """Convert InChI to CTAB."""
        data = {"inchi": inchi}
        return self._make_request("inchi2ctab", data=data)
    
    def inchi_to_inchikey(self, inchi: str) -> dict:
        """Convert InChI to InChIKey."""
        data = {"inchi": inchi}
        return self._make_request("inchi2inchikey", data=data)
    
    def inchi_to_svg(self, inchi: str, size: int = 300) -> dict:
        """Convert InChI to SVG vector graphic."""
        data = {"inchi": inchi, "size": str(size)}
        return self._make_request("inchi2svg", data=data)
    
    def smiles_to_2d(self, smiles: str) -> dict:
        """Generate 2D coordinates from SMILES using Schrodinger's coordgen."""
        data = {"smiles": smiles}
        return self._make_request("smiles22D", data=data)
    
    def smiles_to_ctab(self, smiles: str) -> dict:
        """Convert SMILES to CTAB."""
        # The API expects the SMILES in the request body
        data = {"smiles": smiles}
        return self._make_request("smiles2ctab", data=data)
    
    def smiles_to_inchi(self, smiles: str) -> dict:
        """Convert SMILES to InChI."""
        data = {"smiles": smiles}
        return self._make_request("smiles2inchi", data=data)
    
    def smiles_to_inchikey(self, smiles: str) -> dict:
        """Convert SMILES to InChIKey."""
        data = {"smiles": smiles}
        return self._make_request("smiles2inchikey", data=data)
    
    def smiles_to_svg(self, smiles: str, size: int = 300) -> dict:
        """Convert SMILES to SVG vector graphic."""
        data = {"smiles": smiles, "size": str(size)}
        return self._make_request("smiles2svg", data=data)
    
    def get_structural_alerts(self, molfile_content: str) -> dict:
        """Get structural alerts for a compound."""
        files = {"file": ("molecule.mol", molfile_content, "chemical/x-mdl-molfile")}
        return self._make_request("structuralAlerts", files=files)
    
    def get_status(self) -> dict:
        """Get ChEMBL Beaker status information."""
        return self._make_request("status")


def _format_chembl_response(response: dict, operation: str) -> str:
    """Format ChEMBL Beaker API response into readable output."""
    if not response:
        return f"Error: No response received from ChEMBL Beaker for {operation}"
    
    if "error" in response:
        return f"ChEMBL Beaker Error for {operation}: {response['error']}"
    
    # Handle different response types
    if "result" in response:
        result = response["result"]
        if isinstance(result, str):
            return f"ChEMBL Beaker {operation} Result:\n{result}"
        else:
            return f"ChEMBL Beaker {operation} Result:\n{str(result)}"
...
def standardize_chemical_structure(smiles_or_molfile: str, operation: str = "standardize") -> str:
    """
    Standardize chemical structures using ChEMBL Beaker.
    
    Parameters
    ----------
    smiles_or_molfile : str
        Chemical structure in SMILES or MOL file format
    operation : str, optional
        Standardization operation to perform:
        - "standardize": Standardize molecule using ChEMBL Structure Pipeline
        - "check": Check molecule for issues
        - "exclude": Check if structure will be excluded from ChEMBL
        - "getParent": Remove salt/solvates
        Default is "standardize"
    
    Returns
    -------
    str
        Research log with standardization results
    """
    from datetime import datetime
    
    # Initialize research log
    log = "ChEMBL Beaker Chemical Structure Standardization\n"
    log += "=" * 50 + "\n"
    log += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    
    # Validate operation
    valid_operations = ["standardize", "check", "exclude", "getParent"]
    if operation not in valid_operations:
        return f"Error: Invalid operation '{operation}'. Valid operations: {', '.join(valid_operations)}"
    
    try:
        client = ChEMBLBeakerClient()
        
        # Determine input format and convert if necessary
        if "SMILES" in smiles_or_molfile or " " in smiles_or_molfile:
            # Likely SMILES format, convert to MOL file first
            log += "Input detected as SMILES format. Converting to MOL file...\n"
            mol_response = client.smiles_to_ctab(smiles_or_molfile)
            
            if "error" in mol_response or not mol_response.get("result"):
                return f"Error converting SMILES to MOL file: {mol_response.get('error', 'Unknown error')}"
            
            molfile_content = mol_response["result"]
            log += "Successfully converted SMILES to MOL file format.\n\n"
        else:
            # Assume MOL file format
            molfile_content = smiles_or_molfile
            log += "Input detected as MOL file format.\n\n"
        
        # Perform the requested operation
        log += f"Performing {operation} operation...\n"
        
        if operation == "standardize":
            response = client.standardize_structure(molfile_content)
        elif operation == "check":
            response = client.check_structure(molfile_content)
        elif operation == "exclude":
            response = client.exclude_structure(molfile_content)
        elif operation == "getParent":
            response = client.get_parent_structure(molfile_content)
        
        # Format and return results
        result = _format_chembl_response(response, operation)
        log += result
        
        return log
        
    except Exception as e:
        return f"Error during ChEMBL Beaker {operation}: {str(e)}"


def calculate_physicochemical_properties_chembl(smiles_or_molfile: str, descriptor_type: str = "all") -> str:
    """
    Calculate physicochemical properties using ChEMBL Beaker.
    ...
    descriptor_type : str, optional
        ...
        - "chembl": Selected descriptors available in ChEMBL
        - "all": All RDKit descriptors
        Default is "all"
    
    Returns
    -------
    str
        Research log with calculated properties
    """
    from datetime import datetime
    
    # Initialize research log
    log = "ChEMBL Beaker Physicochemical Property Calculation\n"
    log += "=" * 50 + "\n"
    log += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    try:
        ...
        log += f"Calculating {descriptor_type} descriptors...\n"
        
        if descriptor_type == "chembl":
            response = client.get_chembl_descriptors(molfile_content)
        else:
            response = client.get_all_descriptors(molfile_content)
        
        # Format and return results
        result = _format_chembl_response(response, f"{descriptor_type} descriptors")
        log += result
        
        return log
        
    except Exception as e:
        return f"Error during ChEMBL Beaker property calculation: {str(e)}"


def convert_chemical_format(input_structure: str, input_format: str, output_format: str, size: int = 300) -> str:
    """
    Convert chemical structures between different formats using ChEMBL Beaker.
    
    Parameters
    ----------
    input_structure : str
        Chemical structure in the input format
    input_format : str
        Input format ("smiles", "inchi", "ctab", "molfile")
    output_format : str
        Output format ("smiles", "inchi", "inchikey", "ctab", "svg", "smarts")
    size : int, optional
        Size for SVG output (default: 300)
    
    Returns
    -------
    str
        Research log with conversion results
    """
    from datetime import datetime
    
    # Initialize research log
    log = "ChEMBL Beaker Chemical Format Conversion\n"
    log += "=" * 40 + "\n"
    log += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    
    # Validate formats
    valid_input_formats = ["smiles", "inchi", "ctab", "molfile"]
    valid_output_formats = ["smiles", "inchi", "inchikey", "ctab", "svg", "smarts"]
    
    if input_format not in valid_input_formats:
        return f"Error: Invalid input format '{input_format}'. Valid formats: {', '.join(valid_input_formats)}"
    
    if output_format not in valid_output_formats:
        return f"Error: Invalid output format '{output_format}'. Valid formats: {', '.join(valid_output_formats)}"
    
    try:
        client = ChEMBLBeakerClient()
        
        log += f"Converting from {input_format.upper()} to {output_format.upper()}...\n\n"
        
        # Handle different input formats
        if input_format == "smiles":
            if output_format == "ctab":
                response = client.smiles_to_ctab(input_structure)
            elif output_format == "inchi":
                response = client.smiles_to_inchi(input_structure)
            elif output_format == "inchikey":
                response = client.smiles_to_inchikey(input_structure)
            elif output_format == "svg":
                response = client.smiles_to_svg(input_structure, size)
            else:
                return f"Error: Conversion from SMILES to {output_format} not supported"
                
        elif input_format == "inchi":
            if output_format == "ctab":
                response = client.inchi_to_ctab(input_structure)
            elif output_format == "inchikey":
                response = client.inchi_to_inchikey(input_structure)
            elif output_format == "svg":
                response = client.inchi_to_svg(input_structure, size)
            else:
                return f"Error: Conversion from InChI to {output_format} not supported"
                
        elif input_format in ["ctab", "molfile"]:
            if output_format == "smiles":
                response = client.ctab_to_smiles(input_structure)
            elif output_format == "inchi":
                response = client.ctab_to_inchi(input_structure)
            elif output_format == "inchikey":
                response = client.ctab_to_inchikey(input_structure)
            elif output_format == "smarts":
                response = client.ctab_to_smarts(input_structure)
            elif output_format == "svg":
                response = client.ctab_to_svg(input_structure, size)
            else:
                return f"Error: Conversion from {input_format} to {output_format} not supported"
        
        # Format and return results
        result = _format_chembl_response(response, f"{input_format} to {output_format} conversion")
        log += result
        
        return log
        
    except Exception as e:
        return f"Error during ChEMBL Beaker format conversion: {str(e)}"


def generate_molecular_image(smiles_or_molfile: str, output_format: str = "svg", size: int = 300) -> str:
    """
    Generate molecular images using ChEMBL Beaker.
    ...
    -------
    str
        Research log with image generation results
    """
    from datetime import datetime
    
    # Initialize research log
    log = "ChEMBL Beaker Molecular Image Generation\n"
    log += "=" * 40 + "\n"
    log += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    try:
        ...
        # Determine input format and convert if necessary
        if "SMILES" in smiles_or_molfile or " " in smiles_or_molfile:
            # Likely SMILES format
            log += "Input detected as SMILES format.\n"
            
            if output_format == "svg":
                response = client.smiles_to_svg(smiles_or_molfile, size)
            elif output_format == "2d":
                response = client.smiles_to_2d(smiles_or_molfile)
            else:
            ...
            if output_format == "svg":
                response = client.ctab_to_svg(smiles_or_molfile, size)
            elif output_format == "2d":
                response = client.generate_2d_coordinates(smiles_or_molfile)
            else:
                return f"Error: Unsupported output format '{output_format}'"
        
        log += f"Generating {output_format.upper()} image...\n\n"
        
        # Format and return results
        result = _format_chembl_response(response, f"{output_format} image generation")
        log += result
        
        return log
        
    except Exception as e:
        return f"Error during ChEMBL Beaker image generation: {str(e)}"


def analyze_structural_alerts(smiles_or_molfile: str) -> str:
    """
    Analyze structural alerts for a compound using ChEMBL Beaker.
    ...
    -------
    str
        Research log with structural alert analysis
    """
    from datetime import datetime
    
    # Initialize research log
    log = "ChEMBL Beaker Structural Alert Analysis\n"
    log += "=" * 40 + "\n"
    log += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    try:
        ...
        log += "Analyzing structural alerts...\n"
        response = client.get_structural_alerts(molfile_content)
        
        # Format and return results
        result = _format_chembl_response(response, "structural alert analysis")
        log += result
        
        return log
        
    except Exception as e:
        return f"Error during ChEMBL Beaker structural alert analysis: {str(e)}"


def check_chembl_beaker_status() -> str:
    """
    Check the status of ChEMBL Beaker services.
    
    Returns
    -------
    str
        Research log with service status information
    """
    from datetime import datetime
    
    # Initialize research log
    log = "ChEMBL Beaker Service Status Check\n"
    log += "=" * 35 + "\n"
    log += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    try:
        ...
        log += "Checking ChEMBL Beaker service status...\n"
        response = client.get_status()
        
        # Format and return results
        result = _format_chembl_response(response, "service status")
        log += result
        
        return log
        
    except Exception as e:
        return f"Error checking ChEMBL Beaker status: {str(e)}"


def canonicalize_smiles_string(smiles: str) -> str:
    """
    Canonicalize SMILES strings using ChEMBL Beaker.
    ...
    -------
    str
        Research log with canonicalization results
    """
    from datetime import datetime
    
    # Initialize research log
    log = "ChEMBL Beaker SMILES Canonicalization\n"
    log += "=" * 35 + "\n"
    log += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    try:
        ...
        log += f"Canonicalizing SMILES: {smiles}\n"
        response = client.canonicalize_smiles(smiles)
        
        # Format and return results
        result = _format_chembl_response(response, "SMILES canonicalization")
        log += result
        
        return log
        
    except Exception as e:
        return f"Error during ChEMBL Beaker SMILES canonicalization: {str(e)}"
