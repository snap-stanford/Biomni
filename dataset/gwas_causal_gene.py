...
class gwas_causal_gene(base_task):
    def __init__(self, dataset="opentargets", num_samples=100):
        ...
        query_path = (
            "/dfs/user/kexinh/BioAgentOS/data/llm_gwas/data/benchmark_datasets/" + dataset + "_step2.for_llm.tsv"
        )
        answer_path = "/dfs/user/kexinh/BioAgentOS/data/llm_gwas/data/benchmark_datasets/" + dataset + "_step2.labels"
        ...
        self.query = (
            pd.read_csv(query_path, sep="\t").sample(frac=1, random_state=42).reset_index(drop=True)[:num_samples]
        )
        self.answer = (
            pd.read_csv(answer_path, sep="\t").sample(frac=1, random_state=42).reset_index(drop=True)[:num_samples]
        )
    ...
    def get_example(self, index=None):
        ...
        return {
            "prompt": self.prompt.format(trait=q.description, gene_str=q.symbol_gene_string),
    ...
    def output_class(self):
        ...
        class GeneOutput(BaseModel):
            ...
            causal_gene: str | None = Field(description="causal gene in the format of {gene_name}, e.g. {BRCA1}")
