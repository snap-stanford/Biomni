...
def generate_drug_repurposing_questions(output_dir):
    ...
    try:
        ...
        for disease in disease_names:
            ...
            prompt = prompt_template.format(disease=disease, drug_list=", ".join(novel_db_ids))

            questions.append({"task": "drug_repurposing", "prompt": prompt, "answer": answer})
...
def generate_gene_perturb_selection_questions(output_dir):
    ...
    try:
        ...
        for i in range(len(query)):
            ...
            questions.append({"task": "gene_perturb_selection", "prompt": prompt, "answer": answer[i]})
    ...
    for dataset in ["opentargets", "gwas_catalog", "pharmaprojects"]:
        try:
            ...
            for i in range(len(query_df)):
                ...
                all_questions.append({"task": f"gwas_causal_gene_{dataset}", "prompt": prompt, "answer": answer})
...
def generate_gwas_variant_prioritization_questions(output_dir):
    ...
    try:
        ...
        for i in range(len(df)):
            ...
            questions.append({"task": "gwas_variant_prioritization", "prompt": prompt, "answer": q.rsid})
...
def generate_hle_questions(output_dir):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            questions.append({"task": "humanity_last_exam", "prompt": prompt, "answer": row["letter_answer"]})
...
Output format: {{'causal_gene': [gene1]}}
        ...
        for idx in range(len(data)):
            ...
            prompt = task_description.format(
                phenotype_list=", ".join(phenotypes), candidate_genes=", ".join(candidate_genes)
            )

            questions.append(
                {"task": "patient_gene_detection", "prompt": prompt, "answer": str({"true_genes": true_genes})}
            )
...
Output format: {{'disease_name': XXX, 'OMIM_ID': XXX}}
        ...
        for _, row in df_data.iterrows():
            ...
            prompt = task_description.format(phenotype_list=", ".join(phenotypes), candidate_genes=candidate_genes)
