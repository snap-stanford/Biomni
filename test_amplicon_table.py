...
def test_bfb_captured_interval_length_stats():
    ...
    result = query_amplicons(
        classification='BFB',
        csv_path=CSV_PATH,
        select=['Captured interval length']
    )
    ...
    print(f"  BFB size stats - mean: {mean_length:.2e}, median: {median_length:.2e}, min: {min_length:.2e}, max: {max_length:.2e}")
...
def test_classification_distribution():
    ...
    for cls, count in expected.items():
        assert classifications[cls] == count, \
            f"Expected {count} {cls} but got {classifications[cls]}"
