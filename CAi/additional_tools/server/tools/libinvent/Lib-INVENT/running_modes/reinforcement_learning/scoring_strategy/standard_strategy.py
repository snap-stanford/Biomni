...
class StandardScoringStrategy(BaseScoringStrategy):
    ...
    def _apply_reaction_filters(self, molecules: list[Any], final_score: FinalSummary) -> FinalSummary:
        ...
        loggable_component = LoggableComponent(
            name=sf_component.REACTION_FILTERS, component_type=sf_component.REACTION_FILTERS, score=reaction_scores
        )
        component_parameters = ComponentParameters(
            component_type=sf_component.REACTION_FILTERS,
            name=sf_component.REACTION_FILTERS,
            weight=1,
            smiles=[],
            model_path="",
        )
