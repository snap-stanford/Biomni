...
# -*- coding: utf-8 -*-
"""
REINVENT4 De Novo 分子生成 (action: de_novo)
==============================================
约定：
  - cwd 是 Job 沙盒目录，从 params.json 读取参数
  - 将结果写入 result.json
  - 调试信息用 print(..., file=sys.stderr)
"""
...
# 将工具源码目录加入 sys.path，以便导入 reinvent_generator
...
def main():
    ...
    try:
        ...
        print(f"[INFO] de_novo action 收到参数: {list(params.keys())}", file=sys.stderr)

        # 2. 构造 REINVENT4 配置
        num_variants = params.get("num_variants", params.get("num_samples", 100))
        molecules = params.get("molecules", [])
        if not molecules:
            molecules = [{"id": "de_novo_run", "num_variants": num_variants}]
    ...
    if result_payload.get("success"):
        ...
        print(f"[INFO] de_novo 完成！生成了 {count} 个分子。", file=sys.stderr)
    else:
        print(f"[ERROR] de_novo 失败: {result_payload.get('error')}", file=sys.stderr)
